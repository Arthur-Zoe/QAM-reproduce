...
def selected_task_for(domain):
    return 1 if "maze" in domain else 2
...
print(
    "formal selected-task matrix:",
    f"{len(METHODS)} methods x {len(MAIN_DOMAINS)} domains x 1 selected task x {len(SEEDS)} seeds",
    "=",
    len(METHODS) * len(MAIN_DOMAINS) * len(SEEDS),
    "runs",
)
print(
    "debug selected-task matrix:",
    f"{len(METHODS)} methods x {len(MAIN_DOMAINS)} domains x 1 selected task x 1 seed",
    "=",
