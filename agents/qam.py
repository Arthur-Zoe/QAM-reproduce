...
class QAMAgent(flax.struct.PyTreeNode):
    ...
    # 定义critic loss，主要是计算critic的损失函数，包括Q-learning的TD误差，以及一些统计信息
    ...
    def adj_matching(self, obs, rng, flow_steps=None):
        ...
        # 这里的action_dim是指flow policy的输入输出维度，和环境的action_dim可能不同（如果使用了action_chunking的话），但和critic的输入维度是一样的
        # 单步action的维度是self.config['action_dim']，如果使用了action_chunking，那么flow policy的输入输出维度就是单步action维度乘以horizon_length，否则就是单步action维度
    ...
    def actor_loss(self, batch, grad_params, rng):
        ...
        x_0 = jax.random.normal(x_rng, (batch_size, action_dim))    # x_0是从标准正态分布中采样的噪声，作为flow policy的输入
        x_1 = batch_actions     # x_1是来自数据集的真实动作
        t = jax.random.uniform(t_rng, (batch_size, 1))      
        x_t = (1 - t) * x_0 + t * x_1   # x_t是x_0和x_1的线性插值，作为flow policy在时间t的输入
        vel = x_1 - x_0     # 从噪声走向真实动作的方向

        # 这里的pred是flow policy在时间t的输出，应该是一个velocity field，告诉我们在时间t应该朝哪个方向调整输入x_t才能更接近真实动作x_1
        pred = self.network.select('actor_slow')(batch['observations'], x_t, t, params=grad_params)
        # flow_loss是BC flow-matching loss，要求flow policy在时间t的输出pred和从x_0到x_1的方向vel尽可能一致。
        # 这个损失会促使flow policy学会从噪声x_0出发，沿着正确的方向调整，最终到达真实动作x_1。
        ...
        # 先用 flow 生成一条动作生成轨迹，然后用 Q 函数对最终动作的梯度，反向推回整条轨迹，告诉 actor_fast 每个时间点应该怎么修正。
        ...
        # 原本的速度（如果 residual=True 就是 actor_slow 的输出，否则就是 0）
        ...
        if self.config["residual"]:
            ...
            # 让 actor_fast 学到的速度场，相对于 actor_slow 的变化，匹配 adjoint 给出的高 Q 修正方向
    ...
    3. 两者相加
    4. 对所有相关网络参数反向传播
    5. 更新 critic / actor_slow / actor_fast / one_step_actor 等
    6. 软更新 target_critic 和 target_actor_slow """
    @jax.jit
    ...
    2. 读取 observation 维度
    3. 读取 action 维度
    4. 如果 action_chunking=True，就扩展 action 维度
    5. 创建 critic 网络
    6. 创建 actor vector field 网络
    7. 根据 fql_alpha / edit_scale 决定是否创建额外网络
    8. 初始化参数
    9. 创建 optimizer
    10. 设置 target 网络初始参数
    '''
